...
for (let round = 1; round <= NUM_ROUNDS; round++) {
  ...
  for (const proposalKey in proposals) {
    ...
    if (proposal.length === 1) {
      ...
      const pos = proposalKey.split(",").map((x) => parseInt(x));
      minRow = Math.min(pos[0], minRow);
      maxRow = Math.max(pos[0], maxRow);
      minCol = Math.min(pos[1], minCol);
      maxCol = Math.max(pos[1], maxCol);
    }
  }

  // const grid = new Array(maxRow - minRow + 1)
  //   .fill()
  ...
  //     if (elves.has(key)) {
  //       console.log({ r, c }, r + minRow, c + minCol);
  //       grid[r - minRow][c - minCol] = "#";
  //     }
  //   }
  // }
  // for (const line of grid.map((line) => line.join(""))) console.log(line);
}

let numEmptyGroundTiles = 0;
for (let r = minRow; r <= maxRow; r++) {
  for (let c = minCol; c <= maxCol; c++) {
    const key = createKey(r, c);
    if (!elves.has(key)) {
...
console.log(numEmptyGroundTiles);
